...
static uint8_t *prio_encode(const struct chunkID_set *h, uint8_t *buff, int buff_len, int meta_len)
{
  ...
  if (buff_len < h->n_elements * 4 + 16 + meta_len) {
    return NULL;
  }
  for (i = 0; i < h->n_elements; i++) {
    int_cpy(buff + 16 + i * 4, h->elements[i]);
  }

  return buff + 16 + h->n_elements * 4;
}
...
static const uint8_t *prio_decode(struct chunkID_set *h, const uint8_t *buff, int buff_len, int *meta_len)
{
  ...
  if (buff_len != h->size * 4 + 16 + *meta_len) {
  ...
  for (i = 0; i < h->size; i++) {
    h->elements[i] = int_rcpy(buff + 16 + i * 4);
  }
  h->n_elements = h->size;

  return buff + 16 + h->size * 4;
}
