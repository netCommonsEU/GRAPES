...
#pragma message "RTP Multiflow Chunkiser compiling in debug mode"
#endif
...
#define RTP_MULTI_DEFAULT_CHUNK_SIZE 65536
#define RTP_MULTI_DEFAULT_MAX_DELAY (1ULL << (TS_SHIFT-2))  // 250 ms
...
struct chunkiser_ctx {
  ...
  struct rtp_multi_stream streams[RTP_STREAMS_NUM_MAX];  // its len is fds_len/2
...
static void printf_log(const struct chunkiser_ctx *ctx, int loglevel,
  ...
  if (loglevel <= ctx->verbosity && strlen(fmt)) {
    fprintf(s, "RTP Multiflow Chunkiser: ");
    if (loglevel == 0) {
      fprintf(s, "Error: ");
    }
    va_start(args, fmt);
...
static uint64_t rtptontp(const struct chunkiser_ctx *ctx,
                         const struct rtp_multi_stream *stream, uint32_t rtp) {
...
static void rtp_packet_received(struct chunkiser_ctx *ctx, int stream_id, uint8_t *pkt, int size, struct rtp_info *info) {
  ...
  const void *rtp_p;
  int rtp_p_len;
  struct rtp_multi_stream *stream = &ctx->streams[stream_id];
...
static int conf_parse(struct chunkiser_ctx *ctx, const char *config) {
  ...
  ctx->verbosity = 1;
  ctx->rtp_log = 0;
  chunk_size = RTP_MULTI_DEFAULT_CHUNK_SIZE;
  ctx->max_size = chunk_size + UDP_MAX_SIZE;
  ctx->max_delay = RTP_MULTI_DEFAULT_MAX_DELAY;
  ...
  if (cfg_tags) {
    ...
    grapes_config_value_int(cfg_tags, "rtp_multi_log", &(ctx->rtp_log));
    if (ctx->rtp_log) {
    ...
    if (grapes_config_value_int(cfg_tags, "base_in", &ports[0])) {
      ...
      printf_log(ctx, 1, "Opened ports: %i %i %i %i",ports[0],ports[1],ports[2],ports[3]);
    }
    else{
      i=0;
    }
...
static struct chunkiser_ctx *rtp_multi_open(const char *fname, int *period, const char *config) {
...
static void rtp_multi_close(struct chunkiser_ctx  *ctx) {
  int i;
...
const int *rtp_multi_get_fds(const struct chunkiser_ctx *ctx) {
  return ctx->fds;
}


struct chunkiser_iface in_rtp_multi = {
  .open = rtp_multi_open,
  .close = rtp_multi_close,
  .chunkise = rtp_multi_chunkise,
  .get_fds = rtp_multi_get_fds,
};
